...
func main() {
	...
	//result, _ := app.getFromAPI()
	//result, _ := app.getFromAPIWithClient()
	//result, _ := app.signInWithBearerToken()
	//app.clientWithTrace()
...
func (a *App) signInWithBearerToken() ([]byte, error) {
	...
	req.Header.Add("Accept", "applicaion/json")
...
func (a *App) clientWithTrace() {
	...
	req, err := http.NewRequestWithContext(traceCtx, http.MethodGet, "https://httpbin.org/get", nil)
	if err != nil {
		log.Fatal(err)
	}
